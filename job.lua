...
local function isLeo()
    local job = getJob()
    return job and isLeoJob(job.name) or false
end

local function getGrade()
    local job = getJob()
    return (job and tonumber(job.grade)) or 0
end
...
local function applyVehicleExtras(veh, extras)
    if not extras or type(extras) ~= 'table' then return end
    for extraId, enabled in pairs(extras) do
        local idNum = tonumber(extraId)
        if idNum then
            SetVehicleExtra(veh, idNum, not enabled)
        end
    end
end

local function parseJsonMaybe(value)
...
local function getImpoundModelAndProps(row)
    if not row then return nil, nil end

    -- ESX-style schemas often store vehicle props JSON in `vehicle`.
    -- QBCore-style schemas often store model in `vehicle` and props in `mods`.
    local props = nil
    local model = nil

    if type(row.vehicle) == 'table' then
        props = row.vehicle
    elseif type(row.vehicle) == 'string' then
        local maybeProps = parseJsonMaybe(row.vehicle)
        if maybeProps and maybeProps.model then
            props = maybeProps
        else
            model = row.vehicle
        end
    elseif type(row.vehicle) == 'number' then
        model = row.vehicle
    end

    if not props and row.mods then
        if type(row.mods) == 'table' then
            props = row.mods
        elseif type(row.mods) == 'string' then
            props = parseJsonMaybe(row.mods)
        end
    end
...
local function getVehicleLabel(model)
    ...
    if not label or label == 'NULL' then
        return tostring(display or model)
    end
    return label
end
...
CreateThread(function()
    -- Populate initial player data when resource starts.
    while not ESX.PlayerData do
        Wait(250)
    end
    playerData = ESX.GetPlayerData() or ESX.PlayerData or {}
end)
...
local function takeOutImpound(vehicle)
    ...
    local model, props = getImpoundModelAndProps(vehicle)
    if not model then
        notify(locale('error.no_impound'), 'error')
        return
    end

    -- Pass giveKeys=true and vehicle.id separately so keys are granted (keys are optional in ESX)
    local netId = lib.callback.await('qbx_police:server:spawnVehicle', false, model, coords, vehicle.plate, true, vehicle.id)
...
local function takeOutVehicle(vehicleInfo)
    ...
    local netId = lib.callback.await('qbx_police:server:spawnVehicle', false, vehicleInfo, coords, plate, true)
    ...
    if config.vehicleSettings[vehicleInfo] then
        if config.vehicleSettings[vehicleInfo].extras then
            applyVehicleExtras(veh, config.vehicleSettings[vehicleInfo].extras)
        end
...
local function openGarageMenu()
    local authorizedVehicles = config.authorizedVehicles[getGrade()] or config.authorizedVehicles[0] or {}
    local options = {}
...
local function openImpoundMenu()
    ...
    if not result then
        ...
        for _, v in pairs(result) do
            local model = getImpoundModelAndProps(v)
            local enginePercent = round((tonumber(v.engine) or 0) / 10)
            local currentFuel = tonumber(v.fuel) or 0
            local vName = getVehicleLabel(model)
...
local function openEvidenceLockerSelectInput(currentEvidence)
    ...
    -- ox_inventory stashes must exist (RegisterStash / database stashes).
    -- This keeps a deterministic stash id per evidence locker + slot.
    local stashId = ('police_evidence_%s_%s'):format(currentEvidence, slotNumber)
...
local function spawnHelicopter()
    ...
    local netId = lib.callback.await('qbx_police:server:spawnVehicle', false, config.policeHelicopter, coords, 'ZULU'..lib.string.random('1111'), true)
...
local function scanFingerprint()
    ...
    if not playerId then
        return notify(locale('error.none_nearby'), 'error')
    end
...
RegisterNetEvent('police:client:ImpoundVehicle', function(fullImpound, price)
    ...
    then
        ...
        notify(locale('success.impounded'), 'success')
    else
...
function ToggleDuty()
    onDuty = not onDuty
    LocalPlayer.state:set('onDuty', onDuty, true)
    notify(locale(onDuty and 'info.on_duty' or 'info.off_duty'), 'inform')
    ...
    for i = 1, #sharedConfig.locations.duty do
        lib.zones.box({
            ...
            onEnter = function()
                if not isLeo() then return end
                inPrompt = true
                lib.showTextUI(locale(onDuty and 'info.off_duty' or 'info.on_duty'))
                uiPrompt('duty')
            end,
            onExit = function()
                inPrompt = false
...
CreateThread(function()
    ...
    for i = 1, #sharedConfig.locations.trash do
        lib.zones.box({
            ...
            debug = config.polyDebug,
            onEnter = function()
                if not isLeo() or not onDuty then return end
                inTrash = true
                inPrompt = true
