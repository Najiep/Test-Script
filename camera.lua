...
local function handleActiveCamera(camId)
    ...
    elseif camId == 0 then
        ...
        notify(locale('error.no_camera'), 'error')
    end
end

RegisterNetEvent('police:client:ActiveCamera', handleActiveCamera)
RegisterNetEvent('esx_policejob:ActiveCamera', handleActiveCamera)

local function handleDisableAllCameras()
    if GetInvokingResource() then return end
...
RegisterNetEvent('police:client:DisableAllCameras', handleDisableAllCameras)
RegisterNetEvent('esx_policejob:DisableAllCameras', handleDisableAllCameras)

local function handleEnableAllCameras()
    if GetInvokingResource() then return end
    for i = 1, #config.cameras do
...
RegisterNetEvent('police:client:EnableAllCameras', handleEnableAllCameras)
RegisterNetEvent('esx_policejob:EnableAllCameras', handleEnableAllCameras)
...
local function handleSetCamera(key, isOnline)
    if GetInvokingResource() then return end
    if isArrayTable(key) then
        for i = 1, #key do
...
RegisterNetEvent('police:client:SetCamera', handleSetCamera)
RegisterNetEvent('esx_policejob:SetCamera', handleSetCamera)
