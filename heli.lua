...
local lastVehicleInfoUpdate = 0

local function getPoliceJobNames()
    local jobs = config.policeJobs or config.allowedPoliceJobs or config.policeJobNames or config.policeJob
...
local function isPoliceOnDuty()
    if not Framework.isPlayerLoaded() then return false end
    if not Framework.isJob(getPoliceJobNames()) then return false end

    if config.requireOnDuty == true then
        return Framework.isOnDuty()
    end

    local job = Framework.getJob()
    if job and (job.onduty ~= nil or job.duty ~= nil) then
        return Framework.isOnDuty()
    end
...
local function renderVehicleInfo(vehicle)
    ...
    local licensePlate = getVehiclePlate(vehicle)
            ...
            if vehicleLockState == VEHICLE_LOCK_STATE.scanning then
                if scanValue < 100 then
                    ...
                    if scanValue == 100 then
                        ...
                        lastVehicleInfoUpdate = 0 -- Reset update timer when locked
                    end
                    sleep = 10
                end
            elseif vehicleLockState == VEHICLE_LOCK_STATE.locked then
                scanValue = 100
                -- Only update vehicle info every 500ms to prevent UI flickering
                if GetGameTimer() - lastVehicleInfoUpdate >= 500 then
                    renderVehicleInfo(lockedOnVehicle)
                    lastVehicleInfoUpdate = GetGameTimer()
                end
                sleep = 100
            else
                scanValue = 0
